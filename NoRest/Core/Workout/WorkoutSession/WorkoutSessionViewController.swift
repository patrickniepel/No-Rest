...
class WorkoutSessionViewController: NRViewController {
    ...
    private func showAlert(isFinishAlert: Bool, confirmationHandler: (() -> Void)?) {
        ...
        if isFinishAlert {
            ...
            confirmationAction = UIAlertAction(title: "button.title.finish".localized, style: .default, handler: { _ in
                confirmationHandler?()
            })
        } else {
            message = "workout.session.abort.message".localized
            confirmationAction = UIAlertAction(title: "button.title.abort".localized, style: .destructive, handler: { _ in
                confirmationHandler?()
            })
        }
